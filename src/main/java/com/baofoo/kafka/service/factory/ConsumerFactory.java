package com.baofoo.kafka.service.factory;

import com.baofoo.kafka.model.KafkaProperties;
import com.baofoo.kafka.model.TopicInfo;
import com.baofoo.kafka.tools.IPUtil;
...
//        int connectNum = topicInfo.getPartition() / brokerNum ;//每台服务器上的消费者连接数
    ...
    private static Properties getProperty(TopicInfo topicInfo) {
        ...
        props.put("group.id", topicInfo.isUseAsQueue() ? topicInfo.getTopicName() + "_queue" : topicInfo.getTopicName()+"topic_"+ IPUtil.getLocalIP());
